...
export const getClientByUserId = async (req, res) => {
  try {
    const { userId } = req.params; // Se obtiene el ID del usuario de los parámetros de la URL

    const client = await Client.findOne({ user: userId }) // Busca el cliente por el campo 'user' que referencia al User ID
      .populate('user', '-password -__v') // Popula el usuario asociado si necesitas sus datos
      .select('-__v'); // Excluye el campo '__v'

    if (!client) {
      console.log(`Cliente no encontrado para userId: ${userId}`);
      return res.status(404).send({ success: false, message: 'Client not found for this user ID' });
    }

    return res.send({
      message: 'Client found by user ID',
      success: true,
      client
    });
  } catch (err) {
    console.error('ERROR getting client by user ID:', err);
    return res.status(500).send({ success: false, message: 'Error getting client by user ID', error: err });
  }
};
