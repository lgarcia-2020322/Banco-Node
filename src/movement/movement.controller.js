...
export const createMovement = async (req, res) => {
  try {
    ...
    // Validación inicial
    if (!clientId || !type || amount === undefined || !currency) {
    ...
    switch (type) {
      ...
      case 'transfer':
        // Para transferencias:
        // Si el amount es negativo, se descuenta saldo (salida)
        // Si el amount es positivo, se suma saldo (entrada)
        if (amount < 0) {
          if (client.balance < Math.abs(amount)) {
            return res.status(400).json({ message: 'Saldo insuficiente para transferencia' })
          }
          newBalance += amount // amount negativo, resta saldo
        } else {
          newBalance += amount // amount positivo, suma saldo
        }
        break
